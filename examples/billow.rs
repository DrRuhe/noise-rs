...
fn main() {
    ...
    utils::write_example_to_file(&PlaneMapBuilder::<_,3>::new(billow).build(), "billow_perlin.png");

    let billow = Billow::<Worley>::default();

    utils::write_example_to_file(&PlaneMapBuilder::<_,3>::new(billow).build(), "billow_worley.png");

    let billow = Billow::<Billow<Perlin>>::default();

    utils::write_example_to_file(
        &PlaneMapBuilder::<_,3>::new(billow).build(),
